...
#[cfg(test)]
mod tests {
    use crate::agreement::{agree, Algorithm, PrivateKey, PublicKey, UnparsedPublicKey, X25519};
    use crate::encoding::{AsBigEndian, Curve25519SeedBin};
    use crate::{rand, test};
    ...
    fn test_agreement_invalid_keys() {
        ...
        let alg_variants: [&'static Algorithm; 1] = [&X25519];
    ...
    fn agreement_traits() {
        ...
        let private_key = PrivateKey::generate_for_test(&X25519, &rng).unwrap();
        ...
        assert_eq!(
            format!("{:?}", &private_key),
            "PrivateKey { algorithm: Algorithm { curve: Curve25519 } }"
        );

        let ephemeral_private_key = PrivateKey::generate_for_test(&X25519, &rng).unwrap();
        ...
        let pubkey_re = Regex::new(
            "PublicKey \\{ algorithm: Algorithm \\{ curve: Curve25519 \\}, bytes: \"[0-9a-f]+\" \\}",
        )
        .unwrap();
    ...
    fn test_agreement_random_helper(alg: &'static Algorithm) {
        ...
        let key_types_tested = match alg.id {
            ...
            crate::agreement::AlgorithmID::X25519 => 1,
        };
