...
mod prim;
pub use prim::{Core as CryptoCore, CryptoPrimatives};
...
pub fn encrypt_handshake_init(
    ...
    hs.mix_hash::<prim::Core>(&peer.key.as_bytes());

    // -> e: ephemeral keypair generated by caller
    hs.mix_hash::<prim::Core>(&epk_i.as_bytes());
    ...
    hs.mix_chain::<prim::Core>(&epk_i.as_bytes());

    // -> es:
    let k = hs.mix_key_edh::<prim::Core>(esk_i, &peer.key)?;

    // -> s:
    let static_key =
        EncryptedPublicKey::encrypt_and_hash::<prim::Core>(initiator.public_key.as_bytes(), hs, &k);

    // -> ss:
    let k = hs.mix_key_dh::<prim::Core>(&initiator.private_key, &peer.key)?;

    // payload:
    let timestamp = EncryptedTimestamp::encrypt_and_hash::<prim::Core>(now.to_bytes(), hs, &k);
...
pub fn decrypt_handshake_init<'m>(
    ...
    // <- s:
    hs.mix_hash::<prim::Core>(&receiver.public_key.as_bytes());

    // -> e:
    hs.mix_hash::<prim::Core>(&init.ephemeral_key);
    // > In non-PSK handshakes, the "e" token in a pre-message pattern or message pattern
    // > always results in a call to MixHash(e.public_key).
    // > In a PSK handshake, all of these calls are followed by MixKey(e.public_key).
    // As an optimisation, we don't need the key, so mix_chain is more efficient.
    hs.mix_chain::<prim::Core>(&init.ephemeral_key);

    // -> es:
    let epk_i = PublicKey::from_array(&init.ephemeral_key);
    let k = hs.mix_key_dh::<prim::Core>(&receiver.private_key, &epk_i)?;

    unsafe_log!("decrypting static key");
    // -> s:
    let spk_i = init.static_key.decrypt_and_hash::<prim::Core>(hs, &k)?;
    let spk_i = PublicKey::from_array(spk_i);
    unsafe_log!("decrypted public key {:?}", spk_i.as_bytes());

    // -> ss:
    let k = hs.mix_key_dh::<prim::Core>(&receiver.private_key, &spk_i)?;

    unsafe_log!("decrypting payload");
    // payload:
    let _timestamp = *init.timestamp.decrypt_and_hash::<prim::Core>(hs, &k)?;
...
pub fn encrypt_handshake_resp(
    ...
    hs.mix_chain::<prim::Core>(&epk_r.as_bytes());
    hs.mix_hash::<prim::Core>(&epk_r.as_bytes());

    // <- ee
    let epk_i = PublicKey::from_array(&data.0.ephemeral_key);
    hs.mix_edh::<prim::Core>(esk_r, &epk_i)?;

    // <- se
    let spk_i = PublicKey::from_array(&data.0.static_key.msg);
    hs.mix_edh::<prim::Core>(esk_r, &spk_i)?;

    // <- psk
    let k = hs.mix_key_and_hash::<prim::Core>(&peer.preshared_key);

    // payload:
    let empty = EncryptedEmpty::encrypt_and_hash::<prim::Core>([], hs, &k);
...
pub fn decrypt_handshake_resp(
    ...
    // <- ee:
    hs.mix_edh::<prim::Core>(esk_i, &epk_r)?;

    // <- se:
    hs.mix_dh::<prim::Core>(&initiator.private_key, &epk_r)?;
    ...
    unsafe_log!("decrypting payload");
    // payload:
    resp.empty.decrypt_and_hash::<prim::Core>(hs, &k)?;
...
mod tests {
    ...
    use crate::{
        ...
        encrypt_handshake_resp, prim::Core, CookieState, EphemeralPrivateKey, HandshakeState,
        HasMac, Key, StaticInitiatorConfig, StaticPeerConfig,
    };
    ...
    fn handshake() {
        ...
        let (mut ek1, mut dk1) = hs1.split::<Core>(true);
        let (mut ek2, mut dk2) = hs2.split::<Core>(false);
